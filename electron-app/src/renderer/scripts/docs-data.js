...
const TROUBLESHOOT_ENTRIES = [
  ...
  {
    id: 'slow-download',
    title: 'Download is very slow or stuck',
    tags: ['download', 'slow', 'stuck', 'timeout', 'network', 'python', 'postgres', 'git'],
    symptom: 'Python, PostgreSQL, or Git download takes very long, progress bar stuck, or download fails.',
    cause: 'Slow internet, firewall blocking download servers, or VPN/proxy interference. Some corporate networks block python.org or github.com.',
    solution: 'Try: (1) Disconnect VPN and retry. (2) Check firewall/antivirus is not blocking the app. (3) Use a different network. (4) Download the installer manually from python.org or postgresql.org and install, then the app will detect it as "Already installed". (5) For Git clone, the app uses --depth 1 (shallow) to minimize size.',
  },
  {
    id: 'version-incompatible',
    title: 'Odoo version not working on this PC',
    tags: ['version', 'incompatible', 'windows', 'old', 'error', 'crash'],
    symptom: 'Odoo crashes on startup, or Python/PostgreSQL installer fails on this Windows version.',
    cause: 'Some Odoo/Python/PG versions require specific Windows versions. Python 3.12 (Odoo 19) needs Windows 10+. Older Windows may have missing DLLs.',
    solution: 'Try: (1) Update Windows to the latest version. (2) Use an older Odoo version (Odoo 15 with Python 3.10 works on more systems). (3) Install Visual C++ Redistributable from Microsoft. (4) Check the Installation Log for specific error messages.',
  },
  {
    id: 'antivirus-block',
    title: 'Antivirus blocks installation',
    tags: ['antivirus', 'block', 'defender', 'quarantine', 'false positive'],
    symptom: 'Installer exe gets quarantined, or steps fail silently with no error.',
    cause: 'Windows Defender or third-party antivirus flags the downloaded executables (Python, PostgreSQL installers) as potentially unsafe.',
    solution: 'Temporarily disable real-time protection during installation: Windows Security → Virus & Threat Protection → Manage Settings → Turn off Real-time protection. Re-enable after installation completes.',
  },
  {
    id: 'disk-space',
    title: 'Not enough disk space',
    tags: ['disk', 'space', 'full', 'storage', 'no space'],
    symptom: 'Installation fails partway through, or Odoo crashes after running for a while.',
    cause: 'Full install needs ~5GB: Python (~100MB), PostgreSQL (~300MB), Odoo source (~2GB), venv + packages (~2GB). Filestore grows over time.',
    solution: 'Free up disk space. Change Base Directory in Settings to a drive with more space. Odoo filestore (data/) can be moved by changing data_dir in odoo.conf.',
  },
  {
    id: 'multiple-pg-conflict',
    title: 'Multiple PostgreSQL versions conflict',
    tags: ['postgres', 'multiple', 'conflict', 'service', 'port', '14', '16'],
    symptom: 'PostgreSQL detected but wrong version is used, or DB creation fails.',
    cause: 'Multiple PG versions installed (e.g., PG 14 on port 5432, PG 16 on port 5434). The app or Odoo connects to the wrong one.',
    solution: 'Check db_port in odoo.conf matches the correct PostgreSQL version. The app now reads db_port from odoo.conf and starts the matching PG instance. You can have PG 14 and PG 16 running side by side on different ports.',
  },
  {
    id: 'first-start-slow',
    title: 'First Odoo start takes very long',
    tags: ['first', 'start', 'slow', 'loading', 'initialize', 'database'],
    symptom: 'First time starting Odoo, browser keeps loading for 1-5 minutes.',
    cause: 'On first start with a new database, Odoo initializes the database schema, loads base data, and compiles assets. This is normal.',
    solution: 'Wait 1-5 minutes for the first start. Subsequent starts are much faster (~10-30 seconds). Check the Installation Log for progress. If it takes more than 10 minutes, check for errors in the log.',
  },
];
